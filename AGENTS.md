...
```text
输入普通 TypeScript
→ 生成 Blockly 与 n8n 双画布
→ 发现仓库中不存在的函数
→ DeepSeek 按固定格式生成模块
→ 模块加入 catalog 并自动重新导入
→ 页面立即显示新积木、工作流和运行结果
```

通用代码双画布转换器是产品核心；RoboFrame 保持独立插件边界，在通用闭环
跑通后接入，避免把硬件领域逻辑写回通用转换核心。
...
2. 教学可视化和用户可理解性：15%。
3. 最基本的错误可见性：5%。
4. 安全加固、生产级稳定性、审计与长期运维：当前阶段权重为 0；仅在用户
   重新明确要求时进入范围。

### 实施纪律

- 优先连接真实页面、真实按钮和真实运行结果；测试脚本本身不算产品完成。
- 先完成最短端到端路径，再增加案例；避免为演示之外的假设需求建立抽象层。
- DeepSeek 固定使用 `deepseek-v4-flash`、固定提示词和固定 JSON 契约；采用一条
  薄调用链，暂不建设多 Provider、队列、缓存、限流、监控或复杂配置体系。
- 模型结果只经过现有 schema、finalize、catalog 和重新导入所必需的处理；错误
  直接显示摘要并允许再次点击，避免扩展成生产级恢复框架。
- 当前验收只保留：基础代码转换、一个未知函数现场生成、双画布刷新、一次真实
  n8n 执行和正确输出。压力矩阵、并发矩阵、密钥扫描、证据包和逐文件哈希均在
  当前范围之外。
- 已有忽略目录中的测试产物保留即可，后续精力投入真实 UI 闭环。
- 演示服务避开 `8080` 端口。

### 当前完成标准

用户能够在页面中完成以下连续操作，即视为当前阶段完成：选择或粘贴代码，
点击转换，看到 Blockly 与 n8n 双画布；遇到未知函数时点击一次 AI 生成，看到
动态积木自动出现；随后点击运行并得到正确结果。
