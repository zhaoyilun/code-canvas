...
- `createOperationModuleTemplateV1` turns a request into a deterministic JSON shell for an AI or a
  human author. Its `implementationRef` is `null`; the host derives the immutable implementation
  identity with `finalizeOperationModuleSpecV1` after the semantic fields are complete. Its constraints
  describe the target module; they are not a claim about the source call.
- `OperationModuleSpecV1` is re-exported from `@n8n/dual-canvas-operation-runtime`, the single
  contract used by admission, catalog registration, Blockly rendering, compilation, and execution.
  It admits only synchronous, deterministic, side-effect-free JSON-to-JSON expressions.
- `OperationModuleAdmissionV1` binds a final spec back to its scaffold request by `requestRef`,
  qualified name, and arity. Admission also executes every declared test vector through the shared
...
a capability/plugin package. The operation contract contains no executable source or dynamic
