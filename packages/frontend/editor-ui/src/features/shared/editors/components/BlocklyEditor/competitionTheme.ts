...
export type CompetitionBlocklyMode = 'data-transform' | 'robot-skills';
...
/**
 * Keeps Blockly's native renderer while making its visual language match the
 * competition workbench. This theme is presentation-only: workspace state,
 * block definitions, and compiler behavior stay outside this boundary.
 */
export function createCompetitionBlocklyTheme(
	blockly: BlocklyThemeRuntime,
	mode: CompetitionBlocklyMode,
): Theme {
	const isRobotMode = mode === 'robot-skills';
	const themeName = `n8n-competition-${mode}`;
	const palette = isRobotMode
		? {
				primary: '#0f766e',
				primaryLight: '#2dd4bf',
				primaryDark: '#115e59',
