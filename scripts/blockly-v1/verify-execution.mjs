...
if (!rawPath)
	fail(
		'Usage: verify-execution.mjs <execution-or-raw-output> [--workflow=<path>] [--expect-tampered-preview]',
	);
...
if (workflowArgument) {
	...
	assert(
		isRecord(document) && typeof document.id === 'string' && Array.isArray(document.nodes),
	...
	const compiled = compiler.compileBlocklyWorkspace(payload.workspace, payload.operationCatalog);
	...
	if (expectTamperedPreview) {
		...
		assert(
			isRecord(execution.data) && isRecord(execution.data.resultData),
			'tamper evidence is missing execution.data.resultData',
		);
		assert(
			execution.workflowId === document.id,
			'execution.workflowId does not match --workflow document id',
		);
		assert(
			execution.workflowData.id === execution.workflowId,
			'execution.workflowData id does not match execution.workflowId',
		);
		const executedBlocklyNode = findBlocklyNode(
...
function findBlocklyNode(nodes, source) {
	...
	assert(
		typeof blocklyNode.parameters.blocklyPayload === 'string',
		`${source} Blockly payload is missing`,
	);
	return blocklyNode;
}
